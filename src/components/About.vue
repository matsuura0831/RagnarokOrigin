  ...
  <div class="about bg-gray-100">
    <div class="flex flex-col px-10 text-lg">
      <div class="flex flex-col">
        ...
        <Accordion>
          <template #title>装備を選ぶと「～を手動で反映してください」って出たんだけど？</template>
          <template #content>
          <p>
            自分のステータス・武器を見ながら値を入力した際に，当該装備の効果を重複で受けてしまうことを避けるためです．
            ステータス画面に反映されない項目のみ当該欄にて編集できます．
          </p> 
          </template>
        </Accordion>
        ...
        <Accordion :expand="true">
          ...
          <template #content>
          <p>
            まずは自分の入力した値が正しいかどうか，未実装のギア等による効果が現れていないかをご確認ください．見落としがちな点を下記に示します．
            それでも合わない場合は上部の"お問い合わせ"よりご連絡ください．
          </p>
          <ul>
            <li>プロフェッサーのエレメンタルラーニング(効果時間中にステータス画面を確認してみてください)</li>
          </ul>
          <p>
            細かい注意点
            <ul class="list-disc ml-12">
